...
Changes
-------
Version 1.8: Fixed some lap time reporting issues - the new version of R3E inserts -1 into the PreviousLapTime variable if you go off track (it used to just record the lap time). The app now takes account of this; Make 'this is your last lap' message trigger only when crossing the start line for timed races; Added lap time reporting to qual sessions; added position reporting to qual sessions; added gap (front and behind) reporting to race sessions.
