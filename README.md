...
Changes
-------
Version 1.4: Fixed NPE in engine monitor when starting app after starting game; Added pearls of wisdom. This adds some encouragement to some events. There are 'good', 'neutral' and 'must do better' messages. They are triggered by a probability (i.e. if you take the lead there's a 50% chance of a 'good' message). The messages sometimes play before the event they're associated with, sometimes after. There will only ever be 1 event queued at a time and you won't get more than one such event every 40 seconds (so you won't hear a 'good' then shortly after a 'must do better' event). The min time between pearls and is in the config file, and they can also be disabled from there.
